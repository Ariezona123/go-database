...
func (repository *barangRepositoryImpl) Insert(ctx context.Context, barang entity.Barang) (entity.Barang, error) {
	script := "INSERT INTO barang(nama, stok) VALUES (?,?)"
	result, err := repository.DB.ExecContext(ctx, script, barang.Nama, barang.Stok)
	if err != nil {
		return barang, err
	}
...
func (repository *barangRepositoryImpl) FindById(ctx context.Context, id int32) (entity.Barang, error) {
	//TODO implement me
	script := "SELECt id, nama, stok FROM barang WHERE id = ? LIMIT 1"
	...
	if rows.Next() {
		// ada
		rows.Scan(&barang.Id, &barang.Id, &barang.Nama)
		return barang, nil
	} else {
...
func (repository *barangRepositoryImpl) FindAll(ctx context.Context) ([]entity.Barang, error) {
	//TODO implement me
	script := "SELECt id, stok, nama FROM barang"
	...
	for rows.Next() {
		barang := entity.Barang{}
		rows.Scan(&barang.Id, &barang.Nama, &barang.Stok)
...
func (repository *barangRepositoryImpl) Update(ctx context.Context, id int32, barang entity.Barang) (entity.Barang, error) {
	...
	if rows.Next() {
		// yes
		script := "UPDATE barang SET nama = ?, stok = ? WHERE id = ?"
		_, err := repository.DB.ExecContext(ctx, script, barang.Nama, barang.Stok, id)
		if err != nil {
			return barang, err
		}
		barang.Id = id
